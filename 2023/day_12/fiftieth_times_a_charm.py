...
def solve(spring, groups, index, left_to_place):
    ...
    key = (tuple(spring[index:]), left_to_place)
    ...
    if left_to_place == 0 or '?' not in spring:
        cache_[key] = check_valid(spring, groups)
        return cache_[key]
    ...
    for i in range(index, len(spring)):
        if spring[i] == '?':
            ...
            right = spring.copy()
            key = (tuple(spring[index:]), left_to_place)
            cache_[key] = solve(left, groups, i, left_to_place-1) + solve(right, groups, i, left_to_place)
            return cache_[key]
